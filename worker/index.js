...
- ПРИОРИТЕТ category (важнее цвета и пола):
  ...
  3) Явный пол/стиль: «Для девочки» / «Для мальчика» / «Для неё» / «Для него» / «Для мамы» / «Геймерам»
  4) «Универсальные» — если подходит и мальчику и девочке: нейтральные звери/лес/космос без явного пола, смешанные цвета без розового/голубого доминирования, нет имени/принтов «только для девочки|мальчика». НЕ оставляй category пустым
...
- «Фигуры из шаров» — ТОЛЬКО скрутка/лепка из множества шаров, стоящая на полу. НЕ ставь этот тег для фольгированных персонажей (Пикачу, Гонщик, зайчик), баблов, фонтанов и композиций на стене
...
- age_group: ОБЯЗАТЕЛЬНО одно значение из списка по стилю фото/категории (выписка/1 годик → «Для малышей»; герои/цифры/для девочки|мальчика|универсальные → «Для детей»; для неё/него/юбилей → «Для взрослых»). Не оставляй пустым; «Для любого возраста» — только если совсем неоднозначно
...
${image_url
    ? (holidayOnly || boxOnly || bouquetOnly || figuresOnly
      ...
      : (photozoneOnly
        ...
        : 'Фото приложено — ОБЯЗАТЕЛЬНО определи category (с приоритетом «На выписку» при метриках рождения / «добро пожаловать домой»; при нейтральном детском стиле без явного пола — «Универсальные»), age_group, персонажа и тематическую серию по визуальным признакам. target_audience и occasion пустые. Не ставь «Букет из шаров» только из‑за wall_only. Если сомневаешься — confidence medium/low и дай alts. Логотипы магазинов игнорируй. Имена на табличке — пример персонализации, не в title; но дата+вес+рост / выписка → category «На выписку».'))
    : ''}`;
